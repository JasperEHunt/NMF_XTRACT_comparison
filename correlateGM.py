...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 19 04:09:00 2024
Updated on Fri Jul 26 02:28:00 2024 -- efficiency updates after discussion with Shaun Warrington
Updated on Wed Jun 11 15:19:00 2025 -- restructured comments, improved readability

@author: Jasper E. Hunt

CorrelateGM is a tool for calculating the correlations between NMF GM components and tract blueprints identified through traditional atlas-based tractography.
    Supply two scalar CIFTI files (*.dscalar.nii), and the tool will produce a correlation matrix between every NMF GM component and every tract's blueprint,
    allowing the user to determine which component(s) are best correlated with each tract.

CorrelateGM requires the following libraries:
    sys, numpy, nibabel, matplotlib, seaborn

Usage:
    python correlateGM.py <n_components> <NMF_GM_path> <atlas_tractography_path> <save_directory>
...
Example run:
    > python correlateGM.py 50 '${NMFpath}/NMF_GM_50.dscalar.nii' '${XTRACTpath}/myBlueprint.LR.dscalar.nii' '${analysisPath}/results'

CorrelateGM outputs two files:
	- A .csv file containing the correlation matrix. Each column represents a tract identified via atlas-based tractography. Each row
      represents an NMF GM component.
...
atlas_tractography_path = sys.argv[3] # path to DScalar GIFTI file containing atlas-identified GM components
...
# Display warning if the user-entered number of NMF components does not match the number of components in the NMF_GM file
